...
class KeepUnsplashImageAdapter(
...
) : PagingDataAdapter<KeepUnsplashImageItemUiModel, RecyclerView.ViewHolder>(IMAGE_COMPARATOR) {

    companion object {
        private val IMAGE_COMPARATOR = object : DiffUtil.ItemCallback<KeepUnsplashImageItemUiModel>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return when (viewType) {
            ...
            R.layout.item_search_unsplash_image -> KeepUnsplashImageViewHolder(ItemKeepUnsplashImageBinding.inflate(inflater, parent, false))
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (val uiModel = getItem(position)) {
            ...
            is KeepUnsplashImageItemUiModel.KeepUnsplashImageItem -> (holder as KeepUnsplashImageViewHolder).binding.executeAfter {
                lifecycleOwner = holder.itemView.findViewTreeLifecycleOwner()
                viewModel = imageKeepViewModel
                photo = uiModel.unsplashImageItem
